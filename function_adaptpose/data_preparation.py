...
def data_preparation(args, remove_static_joints=True):
    ...
    if args.dataset == 'h36m':
        ...
        dataset = Human36mDataset(
            dataset_path, remove_static_joints=remove_static_joints)
        if args.s1only:
            subjects_train = ['S1']
        else:
            subjects_train = ['S1', 'S5', 'S6', 'S7', 'S8']
        subjects_test = TEST_SUBJECTS  # 目前是S9 S11
    else:
        raise KeyError('Invalid dataset')

    print('==> Loading 3D data from Human36mDataset ...')
    ...
    if args.dataset_target == '3dhp':
        ...
        poses_target_3d = fetch_3dhp_train(
            path.join('data', 'data_'+args.dataset_target + '_gt_train_3d.npz'))

    if args.dataset_target == 'h36m':
        poses_target_2d = poses_train_2d

    if args.dataset_target == '3dpw':
        keypoints_target = np.load(path.join(
            'data', 'data_'+args.dataset_target + '_' + args.keypoints_target + '_train.npz'), allow_pickle=True)
        poses_target_2d = keypoints_target['positions_2d']
        poses_target_2d = np.repeat(poses_target_2d, 50, axis=0)
        poses_target_3d = np.load(path.join('data', 'data_'+args.dataset_target +
                                  '_' + args.keypoints_target + '_train.npz'), allow_pickle=True)
    ...
    if args.dataset_target == 'skii':
        ...
        poses_target_2d /= np.linalg.norm(poses_target_2d,
    ...
    print('pose_target_2d shape:', poses_target_2d.shape)
    ...
    if args.dataset_target == '3dhp' or args.dataset_target == 'h36m':
        ...
        keypoints_test = create_2d_data_3dhp_test(
            path.join('data', 'data_3dhp_' + args.keypoints_target + '_test.npz'))
        mpi3dhp_3d, mpi3dhp_2d = fetch_3dhp(path.join(
            'data', 'data_3dhp_' + args.keypoints_target + '_test.npz'), keypoints_test)
        mpi3dhp_3d, mpi3dhp_2d = np.concatenate(
            mpi3dhp_3d), np.concatenate(mpi3dhp_2d)

        print('Loaded target dataset 3dhp/h36m, test_shape:', mpi3dhp_3d.shape)
        test_loader = DataLoader(PoseBuffer(mpi3dhp_3d, mpi3dhp_2d, pad=args.pad),
                                 batch_size=args.batch_size,
                                 shuffle=False, num_workers=args.num_workers, pin_memory=True)

    elif args.dataset_target == '3dpw':
        keypoints_test = np.load(path.join(
            'data', 'data_'+args.dataset_target + '_' + args.keypoints_target + '_test.npz'))
        ...
        print('Loaded target dataset 3dpw, test_shape:', _3dpw_2d.shape)
        ...
        print('Loaded target dataset skii, test_shape:', skii_2d.shape)
